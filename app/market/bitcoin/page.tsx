...
export default function MarketDetail() {
    ...
    const [bitcoinPriceHistory, setBitcoinPriceHistory] = useState<number[]>([]);
    const [chartViewMode, setChartViewMode] = useState<"bitcoin_price" | "odds">("bitcoin_price");
    ...
    // Fetch Bitcoin price and update history
    const fetchBitcoinPrice = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/markets/bitcoin/price/`);
            if (response.ok) {
                const data = await response.json();
                if (data.current_price) {
                    setBitcoinPriceHistory((prev) => {
                        const updated = [...prev, data.current_price];
    ...
    // Set up interval to fetch Bitcoin price every 5 seconds
    useEffect(() => {
        if (!market) return;
                        ...
                        <div className="bg-muted rounded-2xl p-4">
                            ...
                            {probabilityViewMode === "percentage" ? (
                                ...
                                <div className="space-y-4">
                                    {/* Bitcoin Price & Market Odds Chart - Polymarket Style */}
                                    <div className="bg-background rounded-2xl border border-border overflow-hidden">
                                        {/* Chart Header */}
                                        <div className={isMobile ? "px-4 py-3 border-b border-border" : "px-6 py-4 border-b border-border"}>
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                    <h3 className={isMobile ? "text-xs font-bold text-foreground" : "text-sm font-bold text-foreground"}>
                                                        {chartViewMode === "bitcoin_price" ? "Bitcoin Price" : "Market Odds"}
                                                    </h3>
                                                    <div className="flex gap-1 ml-4">
                                                        <button
                                                            onClick={() => setChartViewMode("bitcoin_price")}
                                                            className={`${isMobile ? "px-2 py-1 text-xs" : "px-3 py-1.5 text-xs"} font-bold rounded transition-all ${
                                                                chartViewMode === "bitcoin_price"
                                                ...
                                                {chartViewMode === "bitcoin_price" ? (
                                                    <>
                                                        ...
                                                        {bitcoinPriceHistory.length > 0 && (() => {
                                                            ...
                                                            const scale = (price: number) => 240 - ((price - scaledMin) / (scaledMax - scaledMin)) * 200;

                                                            return (
                                                                <>
                                                                    {/* Horizontal Grid Lines */}
                                                                    {[40, 80, 120, 160, 200, 240].map((y, idx) => {
                                                                        const priceValue = scaledMax - (idx / 5) * (scaledMax - scaledMin);
                                                                        return (
                                                                            <g key={`price-grid-${y}`}>
                                                                                <line x1="80" y1={y} x2="900" y2={y} stroke="currentColor" strokeWidth="0.5" className="text-border opacity-30" />
                                                                                <text x="70" y={y + 4} textAnchor="end" fontSize={isMobile ? "9" : "11"} className="fill-muted-foreground">
                                                                    ...
                                                                    {[150, 250, 350, 450, 550, 650, 750].map((x) => (
                                                                        <line key={`v-grid-price-${x}`} x1={x} y1="40" x2={x} y2="240" stroke="currentColor" strokeWidth="0.5" className="text-border opacity-20" />
                                                                    ))}
                                                                    ...
                                                                    {(isMobile 
                                                                        ...
                                                                        <text key={`time-price-${label}`} x={x} y="265" textAnchor="middle" fontSize={isMobile ? "9" : "12"} className="fill-muted-foreground">
                                                                            {label}
                                                                        </text>
                                                                    ))}

                                                                    {/* Bitcoin Price Line */}
                                                                    {bitcoinPriceHistory.length >= 2 && (() => {
                                                                        const points = [];
                                                                        const step = Math.max(1, Math.floor(bitcoinPriceHistory.length / 8));
                                                                        for (let i = 0; i < 8; i++) {
                                                                            const idx = Math.min(i * step, bitcoinPriceHistory.length - 1);
                                                                            points.push(bitcoinPriceHistory[idx]);
                                                                        }
                                                                        
                                                                        const pathD = `M 150 ${scale(points[0])} L 250 ${scale(points[1])} L 350 ${scale(points[2])} L 450 ${scale(points[3])} L 550 ${scale(points[4])} L 650 ${scale(points[5])} L 750 ${scale(points[6])} L 850 ${scale(points[7])}`;
                                                                        const areaD = `${pathD} L 850 240 L 150 240 Z`;
                                                                        
                                                                        return (
                                                                            <>
                                                                                <path d={areaD} fill="url(#bitcoinGradient)" />
                                                                                <path d={pathD} stroke="rgb(249, 115, 22)" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                                                                                <circle cx="850" cy={scale(bitcoinPriceHistory[bitcoinPriceHistory.length - 1])} r="5" fill="rgb(249, 115, 22)" stroke="rgb(15, 23, 42)" strokeWidth="2" />
                                                                            </>
                                                                        );
                                                                    })()}
                                                                </>
