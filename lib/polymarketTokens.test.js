...
test('parses token ID arrays stored as JSON strings', () => {
  const market = {
    clobTokenIds: JSON.stringify(['0xSTRINGYESTOKEN', '0xSTRINGNOTOKEN']),
  };

  assert.equal(resolvePolymarketTokenId(market, 'Yes'), '0xSTRINGYESTOKEN');
  assert.equal(resolvePolymarketTokenId(market, 'No'), '0xSTRINGNOTOKEN');
});
